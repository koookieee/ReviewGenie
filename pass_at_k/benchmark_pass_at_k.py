    ...
    async def _setup_environment(self) -> None:
        ...
        if md_content is not None:
            try:
                ...
                # Fallback: upload original latex dir as-is
                try:
                    await self._environment.upload_dir(latex_dir, f"/{workdir.strip('/')}/latex")
                except Exception as e2:
                    logger.warning(f"Fallback latex upload also failed: {e2}")
...
def _load_task_metadata(task_dir: Path) -> dict:
    ...
    # Load the full paper body for the judge. Fast path: use the
    # pre-converted Markdown at latex/template.tex (despite the .tex name,
    # it's the Markdown the agent actually saw). Only invoke pandoc if the
    # pre-converted body is missing or looks like raw LaTeX.
    latex_dir = task_dir / "latex"
    preconverted = latex_dir / "template.tex"
    paper_body = ""
